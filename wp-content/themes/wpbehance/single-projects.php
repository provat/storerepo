...
>>>>>>> parent of 249407e... upload changes
                 ...
                 if(count($resault)){
                   ...
                   if(count($tags_obj)){
                     ...
                     foreach($tags_obj_ AS $tag) {
                       $tag_string.='<li><a href="'.get_permalink($project_id).'?tag='.$tag.'" tag_slug="'.$tag->slug.'">'.$tag.'</a>';
                     }
                ...
                if($tag == '') {
                  ...
                  $sql = "SELECT pi.* FROM ".$wpdb->prefix."portfolio_images pi, ".$wpdb->prefix."posts p, ";
                  $sql.= $wpdb->prefix."terms t, ".$wpdb->prefix."term_relationships tr WHERE t.name = '$tag' AND ";
                  $sql.= "tr.object_id = pi.image_id AND p.ID = pi.image_id AND pi.portfolio_id = $project_id ";
                  $sql.= "ORDER BY pi.order_pos ASC";
