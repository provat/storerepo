...
function appbuddy_login_error( $redirto, $dirfrom, $wp_user ) {
	...
	if( !is_wp_error( $wp_user ) || !$wp_user->get_error_code() || !AppPresser::is_app() ) return $redirto;
		...
		switch( $wp_user->get_error_code() ) {
			...
			default:
			wp_redirect('/?error=login_failed');
...
function appbuddy_post_modal_template() {
	...
	<aside class="io-modal" id="lost-password" tabindex="-1" role="dialog" aria-hidden="true">
		...
		<div class="io-modal-content">
			...
			<p><input type="text" id="code" name="code" value="" placeholder="Code"/></p>
			<p><input type="password" id="pw" name="pw" value="" placeholder="New Password"/></p>
			<p><input type="password" id="pwr" name="pwr" value="" placeholder="Repeat Password"/></p>
		...
		jQuery('body').on('click', '#new-password', function() {
			...
			if ( jQuery('#email').val() === '' ) {
				jQuery('.code-rsp').html('Email required.');
				return false;
			}
		...
		jQuery('body').on('click', '#change-password', function() {
			...
			if ( jQuery('#pw').val() != jQuery('#pwr').val() || jQuery('#pw').val() === '' ) {
					jQuery('.psw-rsp').html('Passwords do not match.');
					return false;
			}
			
			if ( jQuery('#code').val() === '' ) {
					jQuery('.psw-rsp').html('Please enter your reset code.');
					return false;
			}
