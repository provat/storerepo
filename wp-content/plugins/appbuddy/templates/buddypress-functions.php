...
function bp_legacy_theme_ajax_addremove_friend() {
	...
	// Trying to request friendship
		...
		if ( ! friends_add_friend( bp_loggedin_user_id(), $friend_id ) ) {
			...
			echo '<a id="friend-' . esc_attr( $friend_id ) . '" class="remove" rel="remove" title="' . __( 'Cancel Request', 'buddypress' ) . '" href="' . wp_nonce_url( bp_loggedin_user_domain() . bp_get_friends_slug() . '/requests/cancel/' . $friend_id . '/', 'friends_withdraw_friendship' ) . '" class="requested">' . __( 'Cancel Request', 'buddypress' ) . '</a>';
		}
...
function bp_legacy_theme_ajax_messages_send_reply() {
	...
	if ( !empty( $result ) ) {
		...
		<div class="message-box new-message <?php echo $class; ?>">
			<div class="message-content flip">
			<?php echo bp_loggedin_user_avatar( 'type=thumb&width=30&height=30' ); ?>
			<div class="message-metadata">
				<?php do_action( 'bp_before_message_meta' ); ?>

				<strong><a href="<?php echo bp_loggedin_user_domain(); ?>"><?php bp_loggedin_user_fullname(); ?></a></strong>
				<span class="activity"><?php printf( __( 'Sent %s', 'buddypress' ), bp_core_time_since( bp_core_current_time() ) ); ?></span>
...
function appbuddy_localize_gettext( $translated_text, $text, $domain ) {
    ...
    switch ( $translated_text ) {
        ...
        case 'Cancel Friendship Request' :
            ...
            $translated_text = __( 'Cancel Request', 'buddypress' );
            break;
