						...
						**/	
						switch(valType){
							...
							**/								
							...
							case 'username':
							var ck_username = /^[A-Za-z0-9_]{3,10}$/;
							
							
							valmaximum=inp.attr("validation-maximum");
							valminimal=inp.attr("validation-minimum");
							
						
							if(inp.val().match(ck_username)==null){
							  //runCheck(inp, "No special char and space.Char length must be grater than 3 and less than 10");
							}else if(($.trim(inp.val()).length > valminimal) && (inp.val().length < valmaximum)){
							  removeCheck(inp);
							}
							...
							**/								
							case 'match':
							    var matching_value="";
								my_match="#"+inp.attr('validation-match');
							    matching_value=$(my_match).val();
								if((inp.val()) == matching_value){
									removeCheck(inp);
								}else{
									runCheck(inp, valLabel);
								}
							break;
							/**
							* Validation rule: password_new.	
							**/								
							case 'password_new':
								if(inp.val() == obj.find('[data-validation-type="password_new"]').not(inp).val()){
										removeCheck(inp);
									}else{
										runCheck(inp, valLabel);
									}
						...
						if(options.useAjax === true){
						   ...
						   if(obj.attr('dataType')=='json')
							...
							$.ajax({
								...
								url: obj.attr('action'),
								data: obj.serialize(),
								dataType: call_data_type,
                                                       
					           	beforeSend: function (xhr) {
								 //$('.back-loader').show();
                                                                 //var request = new XMLHttpRequest();
                                                               
                                                                 //request.setRequestHeader("Access-Control-Allow-Origin", "https://mb-system-test.midnight-mb.co.uk");
                                                                 xhr.setRequestHeader("X-Alt-Referer", "https://mb-system-test.midnight-mb.co.uk");
